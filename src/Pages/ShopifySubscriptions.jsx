...
const ShopifySubscriptions = () => {
  ...
  useEffect(() => {
    const fetchPageData = async () => {
      try {
        const res = await fetch('/assets/data/shopifysubscriptions/page.json');
        if (!res.ok) throw new Error('Failed to load page data');
        const json = await res.json();
        setPageData(json);
      } catch (error) {
        console.error('Failed to load Shopify Subscriptions page data', error);
        setPageData(null);
      } finally {
        setLoading(false);
      }
...
export default ShopifySubscriptions;
