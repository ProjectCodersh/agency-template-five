...
const ShopifyB2BWholeSale = () => {
  ...
  useEffect(() => {
    const fetchPageData = async () => {
      try {
        const res = await fetch('/assets/data/shopifywholesale/page.json');
        if (!res.ok) throw new Error('Failed to load page data');
        const json = await res.json();
        setPageData(json);
      } catch (error) {
        console.error('Failed to load Shopify B2B Whole Sale page data', error);
        setPageData(null);
      } finally {
        setLoading(false);
      }
...
export default ShopifyB2BWholeSale;
